...
class _CrudUsersPageState extends State<CrudUsersPage> {
  ...
  Widget _row(
      ...
      bool isHeader = false,
      List<Widget Function(BuildContext)> builders = const []}) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          _cell(cells[0]?.toString() ?? '',
              flex: 3, style: isHeader ? textStyleHeader : textStyle),
          _cell(cells[1]?.toString() ?? '',
              flex: 3, style: isHeader ? textStyleHeader : textStyle),
          _cell(cells[2]?.toString() ?? '',
              flex: 2, style: isHeader ? textStyleHeader : textStyle),
          _cell(cells[3]?.toString() ?? '',
              flex: 2,
              style: isHeader ? textStyleHeader : textStyle,
              custom:
                  builders.isNotEmpty ? builders[0](context) : null),
        ],
      ),
    );
  }
  ...
  Widget _cell(String text,
      {required int flex, TextStyle? style, Widget? custom}) {
    return Expanded(
      flex: flex,
      child: custom ??
